...
const { read, load, lint, json } = require("./snowboard");

const {
  lint: renderLint,
  list: renderList,
  htmlBundle: renderHtmlBundle
...
function watchBuilder(input) {
  const options = {
    persistent: true
  };

  if (commander.watchInterval) {
    options.usePolling = true;
    options.interval = parseDuration(commander.watchInterval);
  }
...
async function watchCmd(input, cmd, fn) {
  ...
  if (commander.watch) {
    const watcher = watchBuilder(input);
    watcher.on("change", () => fn(input, cmd));
  }
}

async function watchHttpCmd(input, cmd, fn) {
  let server = await fn(input, cmd);
  ...
  if (commander.watch) {
    ...
    watcher.on("change", async function() {
      ...
      console.log("-".repeat(64));
      server.close();
      server = await fn(input, cmd);
    });
  }
}

function apibCmd(input, cmd) {
  watchCmd(input, cmd, apibExec);
}

function apibExec(input, cmd) {
  try {
...
function jsonCmd(input, cmd) {
  watchCmd(input, cmd, jsonExec);
}

async function jsonExec(input, cmd) {
  try {
...
function lintCmd(input, cmd) {
  watchCmd(input, cmd, lintExec);
}

async function lintExec(input, cmd) {
  try {
...
function listCmd(input, inputs, cmd) {
  ...
  watchCmd(input, cmd, () => listExec(inputs));
}

async function listExec(inputs) {
  try {
...
async function mockCmd(input, inputs, cmd) {
  inputs.unshift(input);
  watchHttpCmd(input, cmd, (_, cmd) => mockExec(inputs, cmd));
}

async function mockExec(inputs, cmd) {
  try {
    const app = express();
...
async function htmlExec(input, cmd) {
  try {
    ...
    const data = await renderHtmlBundle(result, cmd);
...
async function httpCmd(input, cmd) {
  watchHttpCmd(input, cmd, httpExec);
}

async function httpExec(input, cmd) {
  cmd.output = await tmpDirAsync();

  const result = await load(input);
  const output = await renderHtmlBundle(result, cmd);

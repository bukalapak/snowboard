...
const { read, load: loadStrict, loadLoose, lint } = require("./snowboard");
...
function load(cmd, input, options = {}) {
  if (cmd.loose) {
    return loadLoose(input, options);
  }

  return loadStrict(input, options);
}
...
async function jsonCmd(input, cmd) {
  const result = await load(cmd, input);
...
async function listCmd(inputs, cmd) {
  const items = await Promise.all(inputs.map(val => load(cmd, val)));

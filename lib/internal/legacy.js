    ...
    return {
      ...
      transactions: action.transactions.map(transaction => {
        return {
          ...
          nestedTitle: [tag.title, child.title, action.title, transaction.title].join(' / '),
          request: {
            ...
            messageBody: stringify(transaction.request.example, transaction.request.contentType),
            messageBodySchema: stringify(transaction.request.schema, "application/json"),
          },
          response: {
            ...
            messageBody: stringify(transaction.response.example, transaction.response.contentType),
            messageBodySchema: stringify(transaction.response.schema, "application/json"),
          }
        }
      })
    }
...
function adjust(result) {
  ...
  sortBy(result.tags, 'title').forEach(tag => {
    ...
    resourceGroups.push({
      ...
      resources: tag.children.map(child => {
        return {
          title: child.title,
          nestedTitle: [tag.title, child.title].join(' / '),
          description: child.description,
          transitions: adjustActions(result.actions.filter(action => {
            return isEqual(parameterize(action.tags.join('/')), parameterize([tag.title, child.title].join('/')))
          }), tag, child),
        }
      }),
    });
